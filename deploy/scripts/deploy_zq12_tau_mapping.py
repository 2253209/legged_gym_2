...
from deploy.utils.ankle_joint_converter import convert_pv_joint_2_ori, convert_p_ori_2_joint, convert_ankle_tau_2_motor
from deploy.utils.logger import SimpleLogger, get_title_long, get_title_tau_mapping
...
class Deploy:
    ...
    def ankle_tau_cal(self, ankle_pos_cmd, ankle_pos_cur, ankle_vel_cur):
        tau_right_pitch = self.pd_control(ankle_pos_cmd[0],ankle_pos_cur[0],self.cfg.robot_config.kps[4], 0.0, ankle_vel_cur[0], self.cfg.robot_config.kds[4])
        tau_right_roll = self.pd_control(ankle_pos_cmd[1], ankle_pos_cur[1], self.cfg.robot_config.kps[5], 0.0, ankle_vel_cur[1], self.cfg.robot_config.kds[5])
        tau_left_pitch = self.pd_control(ankle_pos_cmd[2], ankle_pos_cur[2], self.cfg.robot_config.kps[10], 0.0, ankle_vel_cur[2], self.cfg.robot_config.kds[10])
        tau_left_roll = self.pd_control(ankle_pos_cmd[3], ankle_pos_cur[3], self.cfg.robot_config.kps[11], 0.0, ankle_vel_cur[3], self.cfg.robot_config.kds[11])

        return np.array([tau_right_pitch, tau_right_roll, tau_left_pitch, tau_left_roll])
    def run_robot(self):
        # 从真机获取和发送给真机的值
        ...
        try:
            ...
            while key_comm.listening:
                ...
                if key_comm.stepCalibrate:
                    ...
                    action_robot = action_1.copy() # ankle pos_cmd

                    ankle_pos_cmd = action_robot[[4, 5, 10, 11]]
                    ankle_pos_cur = pos_net[[4, 5, 10, 11]]
                    ankle_vel_cur = vel_net[[4, 5, 10, 11]]

                    ankle_tau_cmd = self.ankle_tau_cal(ankle_pos_cmd, ankle_pos_cur, ankle_vel_cur)
