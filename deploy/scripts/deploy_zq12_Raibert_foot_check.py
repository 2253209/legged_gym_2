...
class Deploy:
    def __init__(self, cfg: DeployCfg, path):
        ...
        self.pitch_angle = self.cfg.env.default_dof_pos[10]
        self.roll_angle = self.cfg.env.default_dof_pos[11]
    ...
    def combine_obs_net(self, phase, omega, eu_ang, pos, vel, action):
        ...
        self.obs_net[0, 11:23] = pos # (pos - self.cfg.env.default_dof_pos) * self.cfg.normalization.obs_scales.dof_pos  # 10
    ...
    def run_robot(self):
        ...
        try:
            ...
            while key_comm.listening:
                ...
                vel_net[[10, 11]] = self.pitch_angle, self.roll_angle
                ...
                if key_comm.stepCalibrate:
                    ...
                    # sin wave for ankle mapping check
                    action_robot = np.array(self.cfg.env.default_joint_pos)
                    period = 5
                    amp = 0.2
                    cur_time = (key_comm.timestep - 1) * 0.01
                    self.pitch_angle = amp * np.sin(2 * np.pi * cur_time) + self.cfg.env.default_dof_pos[10]
                    self.roll_angle = amp * np.sin(2 * np.pi * cur_time) + self.cfg.env.default_dof_pos[11]

                    p1, p2, p3, p4 = (
                        convert_p_ori_2_joint(0.0, 0.0, self.pitch_angle, self.roll_angle))

                    action_robot[[10, 11]] = p3, p4
