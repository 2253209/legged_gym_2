...
from deploy.utils.logger import SimpleLogger, get_title_long, get_title_ankle_follow_check
...
class Deploy:
    def __init__(self, cfg: DeployCfg, path):
        ...
        self.obs_robot = np.zeros((1, cfg.env.num_obs_robot + 8), dtype=np.float32)
        self.ankle_joint_cmd = np.zeros(4, dtype=np.float32)
        self.ankle_joint_state = self.cfg.env.default_dof_pos[[4, 5, 10, 11]]
    ...
    def combine_obs_real(self, pos, vel, action, a_0, ankle_cmd, ankle_state):
        self.obs_robot[0, 0:12] = pos
        self.obs_robot[0, 12:24] = vel
        self.obs_robot[0, 24:36] = action
        self.obs_robot[0, 36:48] = a_0
        self.obs_robot[0, 48:52] = ankle_cmd
        self.obs_robot[0, 52:56] = ankle_state
    ...
    def run_robot(self):
        ...
        sp_logger = SimpleLogger(f'{LEGGED_GYM_ROOT_DIR}/logs/dep_log', get_title_ankle_follow_check())
        ...
        try:
            ...
            while key_comm.listening:
                ...
                omega, eu_ang, pos_robot, vel_robot = self.get_obs(es)
                ...
                try:
                    ...
                    self.ankle_joint_state = pos_net[[4, 5, 10, 11]]
                ...
                self.combine_obs_real(pos_robot, vel_robot, action_robot, action_0, self.ankle_joint_cmd, self.ankle_joint_state)
                ...
                if key_comm.stepCalibrate:
                    ...
                    self.ankle_joint_cmd = action_robot[[4, 5, 10, 11]]
